...
## Recent Changes
- 2026-03-03: **Forwarded pump hype fix** — Fixed three issues causing forwarded pump hype messages to evade detection: (1) Cyrillic "х" evasion — multiplier regex `\d+x\b` now uses `[xхΧχ×](?=\s|$|[^\w])` to catch Cyrillic х, Greek Χ/χ, and multiplication sign ×; (2) Added "don't miss out" as distinct FOMO trigger (separate from removed standalone "don't miss"); (3) Added "printing money", "gonna print", "plays loading" as pump hype language; (4) Forwarded messages now flagged with single signal (multiplier OR pump hype OR FOMO) instead of requiring 2+ signals.
- 2026-03-03: **Wallet buying scam: word order & DM fix** — Fixed "I need a old wallet...pay 5.3sol" evasion: (1) Added "need/want + old/aged + wallet" adjective-before-noun pattern; (2) Increased `.{0,30}` → `.{0,60}` distance for "old wallet → pay/sol"; (3) New raw-text pattern for "need/want wallet...pay" + digit+crypto (e.g., "5.3sol") since normalizer destroys digits; (4) `hasAggressiveDmSpam` now also tests raw text to catch "that.Dm" where normalizer merges "that.Dm" → "thatDm" hiding the DM keyword.
- 2026-03-03: **Giveaway scam: digit normalization fix** — Fixed "First 5 lucky members to DM gets $2 SOL" evasion: normalizer converts digits (5→s, 2→not matched), breaking `first\s*\d+` patterns. All `first\s*\d+` giveaway patterns now run on raw text. Added "lucky" to member list. New pattern: `first\s*\d+.{0,40}(dm|pm)` + `$?\d+\s*(sol|eth)` on raw text for "first N...DM...crypto amount". Added `$?\d+` prefix to crypto amount patterns to catch "$2 SOL". Added "gets" to get/receive/claim/win list. Updated both main detection and `runDeterministicScamCheck()`.
...
- 2026-03-01: **Scam detection: fake crypto giveaway** — New patterns catch "DM + giveaway + crypto amount" (e.g., "Frist to DM get 5sol just doing a giveaway"). Crypto amounts like "5sol" matched on raw text since normalizer destroys digits (5→s). Covers: "first/frist to DM" + crypto, "giveaway" + DM + crypto, "free/giveaway" + digit+crypto, "DM to get/claim/win" + digit+crypto. Added to both main detection and `runDeterministicScamCheck()`.
- 2026-02-27: **Wallet buying scam detection fix** — `wall+et` regex now catches typo evasion ("walllet"), `transactions?` handles plural. New patterns: "need [crypto] wallet + pay/write me", "wallet that has transactions". `/report` now runs deterministic scam checks before AI — if regex catches scam, deletes immediately. If AI returns UNKNOWN, defaults to deletion when user reported. Also added `runDeterministicScamCheck()` helper covering wallet buying, airdrop links, exchange impersonation, DM solicitation, channel management, and crypto doubling scams.
- 2026-02-27: **Scam detection: fake exchange listing impersonation fix** — Fixed `\b` word boundary bug killing prefix matching (representat→representative). Added WEEX, Phemex, Upbit, Bithumb, Bitfinex to exchange list. New signals: "official representative of [exchange]", "listing proposal/cooperation", "verify...bio" + exchange name. Added "whom should I contact" to soft collaboration detection.
- 2026-02-27: **FOMO urgency fixes** — Added consecutive 🔥🔥, "make sure...don't miss", "in private...100x". Removed standalone "don't miss" from FOMO (was causing false positives on "don't miss the community call"). Fixed "coming now" urgency.
