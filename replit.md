...
**Master Agent (Agent-to-Agent API)**: An autonomous agent layer (`server/agent/`) that exposes TeliGent's scam detection and threat intelligence as public API services for other agents. Includes Locus payment integration on Base (USDC) for paid service calls, Self Protocol proof-of-human identity on Celo, and OpenServ marketplace integration for multi-agent discoverability. Endpoints: `/api/agent/identity` (full manifest with OpenServ-compatible fields — pricing, payment, trust, endpoints), `/api/agent/services/threat-check` (scam analysis), `/api/agent/services/community-health` (aggregated stats), `/api/agent/wallet/status` (Locus wallet), `/api/agent/openserv/invoke` (OpenServ capability invocation), `/api/agent/openserv/health` (OpenServ health), `/.well-known/agent.json` (agent card discovery). All routes on main server port 5000. Service requests logged in `agent_service_logs` table. Dashboard at `/agent` in the frontend.
