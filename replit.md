...
**Bankr Crypto Intelligence**: Optional per-bot feature that integrates the Bankr REST API (`api.bankr.bot`) for real-time crypto data. When enabled, bots gain a `/price <token>` command and the AI response pipeline is enriched with live token/market data for crypto-related questions. Configured via `bankrEnabled` and `bankrApiKey` columns on `bot_configs` (in Drizzle schema). Service module at `server/telegram/bankr.ts`. Env: `BANKR_API_KEY` (platform-level default). Official Bankr skill installed at `.local/skills/bankr/` with references for agent profiles and LLM gateway. TeliGent is registered as an approved agent on Bankr's agent directory (slug: `builderscout`, profile ID: `69a858d676a35f9fc91b7ec3`).
...
- **Bankr API**: Crypto trading and wallet API (`api.bankr.bot`). Used for real-time token price lookups and crypto market data in community bots. Platform-level key via `BANKR_API_KEY` env var, with per-bot override support.