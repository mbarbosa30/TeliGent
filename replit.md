...
**Bankr Crypto Intelligence**: Optional per-bot feature that integrates the Bankr REST API (`api.bankr.bot`) for real-time crypto data. When enabled, bots gain a `/price <token>` command and the AI response pipeline is enriched with live token/market data for crypto-related questions. Configured via `bankr_enabled` and `bankr_api_key` columns on `bot_configs` (migration-managed, not in Drizzle schema). Service module at `server/telegram/bankr.ts`. Env: `BANKR_API_KEY` (platform-level default).
