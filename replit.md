...
**Master Agent (Agent-to-Agent API)**: An autonomous agent layer (`server/agent/`) that exposes TeliGent's scam detection and threat intelligence as public API services for other agents. Includes Locus payment integration on Base (USDC) for paid service calls, Self Protocol proof-of-human identity on Celo, and OpenServ marketplace integration for multi-agent discoverability. Endpoints: `/api/agent/identity` (manifest), `/api/agent/services/threat-check` (scam analysis), `/api/agent/services/community-health` (aggregated stats), `/api/agent/wallet/status` (Locus wallet). OpenServ agent runs on port 7378 with 3 capabilities (threat-check, threat-check-ai, community-health). Service requests are logged in `agent_service_logs` table. Dashboard at `/agent` in the frontend.
...
- **OpenServ**: Multi-agent marketplace integration. TeliGent registers as a discoverable agent on the OpenServ platform with 3 capabilities (threat-check, threat-check-ai, community-health). Implemented as a lightweight Express server (OpenServ-compatible protocol) on port 7378 alongside the main server. Serves `/.well-known/agent.json` for agent card discovery. Env: `OPENSERV_API_KEY`.