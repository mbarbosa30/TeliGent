...
**Helixa Onchain Identity (Base)**: Read-only integration with the Helixa registry on Base (canonical ERC-8004 contract `0x8004A169FB4a3325136EB29fA0ceB6D2e539a432`). The thin client `server/agent/helixa.ts` wraps the public `https://api.helixa.xyz/api/v2` endpoints (`/stats`, `/agent/:id`, `/agent/:id/cred`, `/name/:name`) with a 5s timeout, jittered retry on 5xx, Zod validation, structured `[helixa]` logs, and a 10-minute cache for network stats. New `bot_configs` columns (`helixa_agent_id`, `helixa_cred_score`, `helixa_cred_tier`, `helixa_profile_url`, `helixa_synced_at`) added via `ensureHelixaColumns` in `server/migrations.ts`. Routes: `GET /api/helixa/network-stats` (public, agent rate-limited), `GET /api/bots/:botId/helixa/status` (owner-gated, refreshes the cred score on read), and `POST /api/admin/bots/:botId/helixa/agent-id` (admin-only setter to attach or clear an agent ID). A background sync `runHelixaSync` runs every `HELIXA_SYNC_INTERVAL_HOURS` (default 6h) and refreshes cred score + tier for all linked bots, emitting `[helixa.sync] refreshed=X skipped=Y failed=Z`. Settings page surfaces a "Helixa Identity (Base)" card with cred score, tier, last sync, and a profile link to `helixa.xyz/agent/:id`. Master Agent page surfaces a "Helixa Network (Base)" panel showing total agents, mint price, and supported frameworks. The per-bot mint flow (SIWA + x402 USDC) is intentionally out of scope here and tracked separately.
